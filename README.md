...
Welcome to **Plot**, a domain-specific language (DSL) for writing type-safe HTML, XML and RSS in Swift. It can be used to build websites, documents and feeds, as a templating tool, or as a renderer for higher-level components and tools. Its primary focus is on static site generation and Swift-based web development.

## Forked

This project is a fork of the original upstream [Plot](https://github.com/JohnSundell/Plot). 
...
Plot requires Swift 6.2 or later. If you’re using a Mac, make sure that `xcode-select` is pointed at an Xcode installation that includes a compatible Swift toolchain.
...
Plot consists of five core parts that together make up both its DSL and its overall document rendering API:
...
Plot was originally written by [John Sundell](https://x.com/johnsundell) as part of the Publish suite of static site generation tools, which is used to build and generate [Swift by Sundell](https://swiftbysundell.com). That suite also includes the Markdown parser [Ink](https://github.com/JohnSundell/Ink), as well as [Publish](https://github.com/JohnSundell/Publish) itself.
